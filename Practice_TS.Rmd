...
fc |> 
  autoplot(
    aus_tourism, level= NULL) +
  labs(
    title = "Aus Domestic Tourism",
    y = "Overnight Trips in millions"
  ) +
  guides(colour = guide_legend(title = "Forecast"))


# extracting models from mable
# using pluck from purrr package
...
additive_components <- components(additive_model)
multiplicative_components <- components(multiplicative_model)

# indiv plots 
p1 <- autoplot(additive_components) +
  labs(title = "Additive ETS Model Components")

p2 <- autoplot(multiplicative_components) +
  labs(title = "Multiplicative ETS Model Components")
...
* Notice that the additive seasonal component sums to approximately zero. The smoothing parameters are  
 α=0.2620 ,  β∗=0.1646,  γ=0.0001 and RMSE=0.4169 .

* Notice that the multiplicative seasonal component sums to approximately  m=4. The smoothing parameters are  α=0.2237 ,  β∗=0.1360 ,  γ=0.0001  and RMSE  =0.4122 .
...
.. ################################################################################################################
    ...
    Use select() when you want to feed the model back into tidy forecasting functions like gg_tsresiduals().

    Use pull() |> pluck(1) when you want to directly manipulate the model, get components, coefficients, or            detailed reports.

.. ################################################################################################################

```{r}

# residuals of additive model

gg_tsresiduals(fit |> select(additive))

# residuals of multiplicative model
gg_tsresiduals(fit |> select(multiplicative))
...
```{r Holt-Winters method with daily data}
...
# The Holt-Winters method can also be used for daily type of data, where the seasonal period is   m
# =7 , and the appropriate unit of time for  h  is in days. Here we forecast pedestrian traffic at a busy Melbourne train station in July 2016.
...
sth_cross_ped <- 
  ...
  index_by(Date) |>                                           # indexing buy date created date groups
  
  summarise(Count = sum(Count)/1000)                          # cums of pass on a day in thousands


sth_cross_ped |>
  
  filter(Date <= "2016-07-31") |>                                   # training model till July End
  
  model(
    hw = ETS(Count ~ error("M") + trend("Ad") + season("M"))        # hots multiplicative damped model
  ) |>
  
  forecast(h = "2 weeks") |>                                        # forecasting Till Ausugt Mid
...
```
###### Estimation and model selection (ETS)
...
# Domestic holiday tourist visitor nights in Australia

tourism               # data for australia
...
components(fit) |>                                              # fit is a mable - model table
  autoplot() +
...
Note Important : Because this model has multiplicative errors, the innovation residuals are not equivalent to the regular residuals (i.e., the one-step training errors). The innovation residuals are given by ^εt, while the regular residuals are defined as  yt−^yt|t−1 . We can obtain both using the augment() function.
...
augment(fit) |>
  pivot_longer(cols = c(".resid", ".innov"), names_to = "type", values_to = "value") |>
...
# p1 <- autoplot(aug, .vars = ".resid") +
#   labs(title = "Regular residuals", y = NULL, x = NULL) +
#   theme_minimal()
# 
# p2 <- autoplot(aug, .vars = ".innov") +
#   labs(title = "Innovation residuals", y = NULL, x = "Quarter") +
#   theme_minimal()
# 
# p1 / p2
...
augment(fit) |>
  ggplot(aes(x = Quarter)) +                                      # x axis
  geom_line(aes(y = .resid, color = "Residuals")) +
  geom_line(aes(y = .innov, color = "Innovations")) +
  labs(
    title = "Residuals and Innovations Over Time",
    x = "Quarter",
    y = "Value"
  ) +
  scale_color_manual(values = c("Residuals" = "blue", "Innovations" = "red")) +
  guides(color = guide_legend(title = NULL))


#########

p1 <- ggplot(aug, aes(x = Quarter, y = .resid)) +
  geom_line(color = "blue") +
  labs(title = "Residuals", y = "Residuals", x = NULL)

# Innovations plot
p2 <- ggplot(aug, aes(x = Quarter, y = .innov)) +
  geom_line(color = "red") +
  labs(title = "Innovations", y = "Innovations", x = "Quarter")
...
plot(aug$Quarter, aug$.resid, type = "l", col = "blue",
     main = "Residuals", ylab = "Residuals", xlab = "")
plot(aug$Quarter, aug$.innov, type = "l", col = "red",
     main = "Innovations", ylab = "Innovations", xlab = "Quarter")
...
augment(fit) |>
  ...
  facet_wrap(~Component, ncol = 1, scales = "free_y",
             labeller = as_labeller(c(
               `.resid` = "Regular residuals",
               `.innov` = "Innovation residuals"
             ))) +
...
fit |>
  ...
  forecast(h = 8) |>                      # gives a fable - forecast table
  
  autoplot(aus_holidays)+                 # plots the original data & forecast by matching against date-time index
                                          # default levels 80 and 95%

  labs(title="Australian domestic tourism", # labels the data
